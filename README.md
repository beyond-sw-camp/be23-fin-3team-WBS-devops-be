...
여러 관리자가 동시에 봐도 부하 없이 즉시 갱신되는 운영 대시보드입니다.

- **Kafka Streams 실시간 집계**: 시간대별 처리량(1h Tumbling Window 25h 보관), 진행 중 작업 수(KTable), 반품 비율(1d Window)을 RocksDB state store에서 **1ms 이하 응답**으로 조회
- **오늘의 우선순위**: 당일 입고·출고·이동·반품·재고 부족·승인 대기 지시서가 카드로 요약, 클릭 시 상세 화면 즉시 이동
- **실시간 알림**: WebSocket/STOMP + Redis Pub/Sub로 다중 인스턴스 환경에서도 동일한 알림이 전 운영자에게 도달
- **회전율·랭킹 분석**: 출고량 Top N, 월별 회전율 추이를 차트로 시각화
